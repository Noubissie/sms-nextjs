...
let StaffGrade = React.forwardRef((props,ref)=>{
    ...
    let validationSchema = Yup.object({
        staffGrade: Yup.array().nullable().of(
            Yup.object({
                grade: Yup.string().nullable().required("PCEG,PLEG,PLET,...")
            })
        )
    })
    let onSubmit = async (values, onSubmitProps) =>{
        ...
        let res = await fetch("/api/generalapi?staffGrade=true",
        {
            method:"POST",
        ...
        toast.success("BasicConfiguration",{staffGrade: toast.POSITION.TOP_CENTER,autoClose:2000})
                    ...
                    <Formik
                        ...
                        {({
                            ...
                            <Card>
                                ...
                                <Card.Body>
                                <FieldArray name="staffGrade">
                                    
                                    {   
                                        (fieldprops,index)=>{
                                            let {push,remove,form} = fieldprops
                                            const {values} = form
                                            const {staffGrade} = values 
                                          return(
                                            <div key={index}>
                                            {
                                                staffGrade.map((value,index)=>{
                                                    return(
                                                        <div key={index}>
                                                            <Row>
                                                                ...
                                                                <Col xs="12">
                                                                    <div className="mb-4">
                                                                        ...
                                                                        <Field
                                                                            ...
                                                                            error={(((errors || {}).staffGrade || {})[index] ||{}).grade  ? true : false }
                                                                            label={errors.hasOwnProperty(`${staffGrade[index].grade}`) && touched.hasOwnProperty(`${staffGrade[index].grade}`) ? errors.staffGrade[index].grade : "staffGrade Name" }
                                                                            {...getFieldProps(staffGrade[index].grade)}
                                                                            value={value.grade}
                                                                            component={TextField}
                                                                            name={`staffGrade[${index}].grade`} 
                                                                            id={`staffGrade[${index}].grade`}
                                                                        ...
                                                                        />
                                                                        {/* <ErrorMessage name={`staffGrade[${index}].grade`}  component={errorFieldMessage}/> */}
                                                                    </div>
                                                                </Col >
                                                            </Row>
